...
impl SGX {
    ...
    fn from_env() -> Result<Self, String> {
        ...
        let enclave_edl_path = env::var("ENCLAVE_EDL_PATH").unwrap_or_else(|_| {
            fs::canonicalize("../enclave/Enclave.edl")
                .expect("fail to canonicalize path")
                .to_str()
                .expect("invalid path")
                .to_string()
        });

        Self::new(&c_sdk_path, is_sim, &rust_sdk_path, enclave_edl_path)
    }
}
...
fn generate_bridge(sgx: &SGX, untrusted_dir: &str) {
    ...
    cmd.args(&["--untrusted", &sgx.enclave_edl_path]);

    let search_paths = format!("{}/include {}/edl", &sgx.sdk_dir, &sgx.rust_sdk_path);
    ...
    let output = cmd.output().expect("fail to generate bridges");
